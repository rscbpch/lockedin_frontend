...
class GroupChatService {
  ...
  Future<Map<String, dynamic>> removeMembers({
    ...
    debugPrint('📡 removeMembers: ${response.statusCode} - ${response.body}');

    if (response.statusCode == 200) {
      return jsonDecode(response.body) as Map<String, dynamic>;
    }
    throw Exception('Failed to remove members: ${response.statusCode} - ${response.body}');
  }
  ...
  Future<void> renameGroup({
    ...
    debugPrint('📡 renameGroup: ${response.statusCode} - ${response.body}');

    if (response.statusCode != 200) {
      throw Exception('Failed to rename group: ${response.statusCode} - ${response.body}');
    }
  }

  /// PATCH /api/groupchat/:groupId/transfer
  Future<void> transferOwnership({
    ...
    final response = await _client
        .patch(
          Uri.parse('${Env.apiBaseUrl}/groupchat/$groupId/transfer'),
          headers: await _headers(),
          body: jsonEncode({'newOwnerId': newOwnerId}),
        )
        .timeout(_timeout, onTimeout: () => throw Exception('Request timed out.'));

    debugPrint('📡 transferOwnership: ${response.statusCode} - ${response.body}');

    if (response.statusCode != 200) {
      final body = jsonDecode(response.body) as Map<String, dynamic>;
      throw Exception(body['message'] ?? 'Failed to transfer ownership: ${response.statusCode}');
    }
  }
  ...
  Future<void> leaveGroup(String groupId) async {
    ...
    debugPrint('📡 leaveGroup: ${response.statusCode} - ${response.body}');

    if (response.statusCode != 200) {
      final body = jsonDecode(response.body) as Map<String, dynamic>;
      throw Exception(body['message'] ?? 'Failed to leave group: ${response.statusCode}');
    }
  }
