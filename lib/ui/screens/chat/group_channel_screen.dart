...
class _GroupChannelScreenState extends State<GroupChannelScreen> with ActivityTracker {
  @override
  Widget build(BuildContext context) {
    final channel = StreamChannel.of(context).channel;
    final groupName = channel.extraData['name'] as String? ?? channel.name ?? 'Group Chat';
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text('${channel.memberCount ?? 0} members', style: const TextStyle(color: AppColors.grey, fontSize: 11)),
                  ],
                ),
              ),
            ],
  ...
  void _openGroupDetails(BuildContext context, Channel channel) {
    ...
    final groupId = channelId.startsWith('group-') ? channelId.replaceFirst('group-', '') : channelId;

    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => GroupDetailsScreen(groupId: groupId, channel: channel),
      ),
    );
  }
}
