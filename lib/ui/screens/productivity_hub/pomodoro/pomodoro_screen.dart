...
class _TimerTextInputFormatter extends TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(TextEditingValue oldValue, TextEditingValue newValue) {
    ...
    if (digitsOnly.isEmpty) {
      return const TextEditingValue(text: '00:00', selection: TextSelection.collapsed(offset: 5));
    }

    final boundedDigits = digitsOnly.length > 4 ? digitsOnly.substring(digitsOnly.length - 4) : digitsOnly;
    ...
    final formatted = '${clampedMinutes.toString().padLeft(2, '0')}:${clampedSeconds.toString().padLeft(2, '0')}';
...
class _PomodoroScreenState extends State<PomodoroScreen> {
  ...
  void _initializeVideo() async {
    try {
      _videoController = VideoPlayerController.asset('assets/images/coffee.mp4');

      _videoController!.addListener(() {
        if (_videoController!.value.hasError) {
          setState(() {
            _videoHasError = true;
            _videoErrorMessage = _videoController!.value.errorDescription ?? 'Unknown video error';
          });
        }
      });
      ...
      if (mounted) {
        _updateVideoPlaybackSpeed(context.read<PomodoroTimerProvider>().totalSeconds);
        setState(() {});
      }
    } catch (e) {
      setState(() {
  ...
  Widget _trackingPanel() {
    return Stack(
      children: [
        ...
        if (_showTracking)
          Positioned.fill(
            child: GestureDetector(onTap: () => setState(() => _showTracking = false), behavior: HitTestBehavior.opaque, child: const SizedBox.expand()),
          ),
              ...
              child: Column(
                children: [
                  ...
                  Padding(
                    ...
                    child: Align(
                      ...
                      child: GestureDetector(
                        ...
                        child: const Icon(Icons.close, size: 32, color: Colors.grey),
                      ),
                    ),
                  ),
                  ...
                  Padding(
                    ...
                    child: Row(children: [_tabBtn(Icons.trending_up_rounded, 0), _tabBtn(Icons.group_rounded, 1)]),
                  ),
                  ...
                  Expanded(
                    child: _loadingTracking
                        ? const Center(child: CircularProgressIndicator(color: Color(0xFF6B3F1A)))
                        : _trackingTab == 0
                        ? _personalTab()
                        : _friendsTab(),
                  ),
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            border: selected ? Border.all(color: Colors.transparent, width: 1) : Border.all(color: AppColors.primary, width: 1),
          ),
          child: Icon(icon, size: 22, color: selected ? Colors.white : Colors.black45),
        ),
      ),
    );
  }
  ...
  Widget _personalTab() {
    if (_stats == null) {
      return const Center(
        child: Text('No stats available', style: TextStyle(color: Colors.black38)),
      );
    }
  ...
  void _syncTimeInputWithProvider(PomodoroTimerProvider provider, {bool force = false}) {
  ...
  void _applyTypedTime(PomodoroTimerProvider provider) {
    ...
    if (parsedSeconds == null) {
      _syncTimeInputWithProvider(provider, force: true);
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Enter time as minutes (e.g. 25) or mm:ss (e.g. 25:00).'), duration: Duration(seconds: 2)));
      return;
    }
